...
class WeightedGCCA(metric):
    ...
    def __init__(self, ds, m_rank, verbose=True):
        ...
        self.V = len(self.F)  # Number of views
        self.k = m_rank  # Dimensionality of embedding we want to learn
        eps = [1.e-8 for i in range(self.V)]#1.e-8
#        viewWts = [0.5 for i in range(3)]
        np.random.seed(666)
        viewWts = [np.random.rand() for i in range(self.V)]
        print(viewWts)
        # some common setting
    ...
    def solve(self):
        ...
        self.list_U = self.U
        # np.save("W_list_wgcca.npy",self.U)
...
#    data.generate_synthetic_dataset()
    ...
    clf.solve()
    e4 = time.time()
    print ("total time_4 is ", e4 - s)
    v1_1_test, v2_1_test, v3_1_test = clf.transform(data.test_data)
    # calculate all kind of metric
#    print("reconstruction error of G in training is: ", clf.cal_G_error(data.train_data, test=False))
#    print("reconstruction error of G in testing is: ", clf.cal_G_error(data.test_data, test=True))
#    print("each view's spare of U is ", clf.cal_spare())
...
#    print("training data Err is: ", clf.cal_err_lkx(data.train_data, test=False))
#    print("testing data Err is: ", clf.cal_err_lkx([v1_test, v2_test,v3_test], test=True))
...
#     main(inPath='.../gcca_data/multi_language/million_multi_lang.pickle',k=4)
    print("total correlation in training data is: ",np.sum(clf.cal_correlation(clf.list_projection)))
    print("total correlation in testing data is: ",np.sum(clf.cal_correlation([v1_1_test, v2_1_test, v3_1_test])))
    print("c_training data ACC is: ",clf.cal_average_precision(clf.list_projection))
    print("c_testing data ACC is: ",clf.cal_average_precision([v1_1_test, v2_1_test, v3_1_test]))
    print("a_training data ACC is: ",clf.cal_acc(clf.list_projection))
    print("a_testing data ACC is: ",clf.cal_acc([v1_1_test, v2_1_test, v3_1_test]))
    print("training data Err is: ", clf.cal_err_lkx(clf.list_projection))

    print("reconstruction error of G in training is: ",clf.cal_G_error(data.train_data, test=False) )#clf.cal_err_lkx(data.train_data, test=False),clf2.cal_err_lkx(data.train_data, test=False)
#    print("reconstruction error of G in testing is: ", clf.cal_err_lkx(data.test_data, test=True),clf2.cal_err_lkx(data.train_data, test=True))
    
    print("each view's sparsity of U is ", clf.cal_spare())
    print("total spasity is: ", clf.cal_spare()[0])
#    print("total spasity is: ", np.mean(clf.cal_spare()[0]),np.mean(clf2.cal_spare()))
